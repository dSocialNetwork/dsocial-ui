...
export const getFeedContent = ({ sortBy = 'trending', category, limit = 20 }) => (
  ...
  dispatch({
    ...
    payload: getDiscussionsFromAPI(sortBy, { tag: category, limit }, dpayAPI),
    meta: {
      sortBy,
...
export const getUserComments = ({ username, limit = 20 }) => (dispatch, getState, { dpayAPI }) =>
  dispatch({
...
export const getReplies = () => (dispatch, getState, { dpayAPI }) => {
  const state = getState();
...
export const getMoreReplies = () => (dispatch, getState, { dpayAPI }) => {
  const state = getState();
  const feed = getFeed(state);
  const posts = getPosts(state);
  const category = getAuthenticatedUserName(state);
...
export const getBookmarks = () => (dispatch, getState, { dpayAPI }) => {
  const state = getState();
  const bookmarks = getBookmarksSelector(state);

  dispatch({
    type: GET_BOOKMARKS.ACTION,
    payload: getBookmarksData(bookmarks, dpayAPI).then(posts =>
