...
import { createPermlink, getBodyPatchIfSmaller } from '../../vendor/dSiteHelpers';
...
const broadcastComment = (
  ...
  if (reward === rewardsValues.none) {
    commentOptionsConfig.max_accepted_payout = '0.000 BBD';
  } else if (reward === rewardsValues.all) {
    commentOptionsConfig.percent_dpay_dollars = 0;
  }
  ...
  return dPayIdAPI.broadcast(operations);
};
...
export function createPost(postData) {
  ...
  return (dispatch, getState, { dPayIdAPI }) => {
    const {
      parentAuthor,
      parentPermlink,
      author,
