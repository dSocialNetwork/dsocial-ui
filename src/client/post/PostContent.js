...
class PostContent extends React.Component {
  ...
  render() {
    ...
    const dsocialHost = appUrl || 'https://dsocial.io';
    let canonicalHost = dSocialHost;

    if (postMetaData && _.indexOf(postMetaData.app, 'dsite') === 0) {
      canonicalHost = 'https://dsite.io';
    }
    ...
    const url = `${dsocialHost}${dropCategory(content.url)}`;
    const ampUrl = `${url}/amp`;
    const metaTitle = `${title} - dSocial`;
    ...
    return (
      <div>
        <Helmet>
          ...
          <meta property="og:site_name" content="dSocial" />
          <meta property="article:tag" content={category} />
          <meta property="article:published_time" content={created} />
          <meta property="twitter:card" content={image ? 'summary_large_image' : 'summary'} />
          <meta property="twitter:site" content={'@dsocialio'} />
