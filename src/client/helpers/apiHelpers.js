...
/** *
 * Get the path from URL and the API object of dPay and return the correct API call based on path
 * @param path - as in URL like 'trending'
 * @param API - the { api } from dPay npm package
 * @param query {Object} - the same query sending to dPay API
 * @param dpayAPI - The same giving to dPay API
 * @returns {function}
 */
export function getDiscussionsFromAPI(sortBy, query, dpayAPI) {
  switch (sortBy) {
    case 'feed':
    case 'hot':
    case 'created':
    ...
    case 'comments':
    case 'promoted':
      return dpayAPI.sendAsync(`get_discussions_by_${sortBy}`, [query]);
    default:
...
export const getAccount = username =>
  DPayAPI.sendAsync('get_accounts', [[username]]).then(result => {
    if (result.length) {
...
export const getFollowingCount = username =>
  DPayAPI.sendAsync('call', ['follow_api', 'get_follow_count', [username]]);
...
export const getAccountHistory = (account, from = -1, limit = defaultAccountLimit) =>
  DPayAPI.sendAsync('get_account_history', [account, from, limit]);

export const getDynamicGlobalProperties = () =>
  DPayAPI.sendAsync('get_dynamic_global_properties', []);
...
export const getAccountReputation = (name, limit = 20) =>
  DPayAPI.sendAsync('call', ['follow_api', 'get_account_reputations', [name, limit]]);
