...
describe('getAppData', () => {
  ...
  it('should return an object with the appName and version', () => {
    const post = { json_metadata: '{ "app": "dSocial/1.2.4" }' };
    expect(getAppData(post)).toEqual({ appName: 'dSocial', version: '1.2.4' });
  });

  it('should return an object with the appName and empty version if version is absent', () => {
    const post = { json_metadata: '{ "app": "dSocial" }' };
    expect(getAppData(post)).toEqual({ appName: 'dSocial', version: '' });
  });

  it('should handle more app parameters without failing, eg. dSocial/1.2/other', () => {
    const post = { json_metadata: '{ "app": "dSocial/1.2.3/something" }' };
    expect(getAppData(post)).toEqual({ appName: 'dSocial', version: '1.2.3' });
  });
});
