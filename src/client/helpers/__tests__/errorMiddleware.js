...
describe('parseBlockChainError', () => {
  it('should fingerprint errors', () => {
    let actual = parseBlockChainError(
      '( now - auth.last_root_post ) > DPAY_MIN_ROOT_COMMENT_INTERVAL: You may only post once every 5 minutes.',
    );
    let expected = 'You may only post once every 5 minutes.';
    expect(actual).toEqual(expected);

    actual = parseBlockChainError(
      '(now - auth.last_post) > DPAY_MIN_REPLY_INTERVAL: You may only comment once every 20 seconds.',
    );
    ...
    expected = 'Your bandwith has been exceeded. Please wait to transact or power up BEX.';
    expect(actual).toEqual(expected);

    actual = parseBlockChainError(
      'info->abs_rshares > DPAY_VOTE_DUST_THRESHOLD || vote_weight == 0: Voting weight is too small, please accumulate more voting power or BEX Power.',
    );
    expected =
      'Your voting power is too small, please accumulate more voting power or BEX Power.';
    expect(actual).toEqual(expected);
