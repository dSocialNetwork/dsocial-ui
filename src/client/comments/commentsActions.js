...
import { createCommentPermlink, getBodyPatchIfSmaller } from '../vendor/dSiteHelpers';
...
export const likeComment = (commentId, weight = 10000, vote = 'like', retryCount = 0) => (
  ...
  dispatch({
    type: LIKE_COMMENT,
    payload: {
      promise: dPayIdAPI.vote(voter, author, permlink, weight).then(res => {
        // reload comment data to fetch payout after vote
        dpayAPI.sendAsync('get_content', [author, permlink]).then(data => {
