...
import { BEX, BBD } from '../../common/constants/cryptos';
...
export default class Transfer extends React.Component {
  ...
  state = {
    currency: Transfer.CURRENCIES.BEX,
    oldAmount: undefined,
  };

  componentDidMount() {
    const { cryptosPriceHistory } = this.props;
    const currentDPayRate = _.get(cryptosPriceHistory, 'BEX.priceDetails.currentUSDPrice', null);
    const currentBBDRate = _.get(cryptosPriceHistory, 'BBD*.priceDetails.currentUSDPrice', null);

    if (_.isNull(currentDPayRate)) {
      this.props.getCryptoPriceHistory(BEX.symbol);
    }

    if (_.isNull(currentBBDRate)) {
      this.props.getCryptoPriceHistory(BBD.symbol);
    }
  }
  ...
  getUSDValue() {
    ...
    if (dpayRateLoading || invalidAmount) return '';

    if (currency === BEX.symbol) {
      amount = parsedAmount * parseFloat(currentDPayRate);
    } else {
      amount = parsedAmount * parseFloat(currentBBDRate);
    }
  ...
  handleContinueClick = () => {
    ...
    form.validateFields({ force: true }, (errors, values) => {
      if (!errors) {
        ...
        const win = window.open(DPayID.sign('transfer', transferQuery), '_blank');
        win.focus();
  ...
  validateUsername = (rule, value, callback) => {
    ...
    dpayAPI.sendAsync('get_accounts', [[value]]).then(result => {
      if (result[0]) {
        callback();
      } else {
        callback([
  ...
  validateBalance = (rule, value, callback) => {
    ...
    const selectedBalance =
      this.state.currency === Transfer.CURRENCIES.BEX ? user.balance : user.bbd_balance;
  ...
  render() {
    ...
    const currencyPrefix = getFieldDecorator('currency', {
      ...
      <Radio.Group onChange={this.handleCurrencyChange} className="Transfer__amount__type">
        <Radio.Button value={Transfer.CURRENCIES.BEX}>{Transfer.CURRENCIES.BEX}</Radio.Button>
        <Radio.Button value={Transfer.CURRENCIES.BBD}>{Transfer.CURRENCIES.BBD}</Radio.Button>
      </Radio.Group>,
    );
    ...
    return (
      ...
      >
        ...
        <FormattedMessage
          ...
          defaultMessage="Click the button below to be redirected to dPayID to complete your transaction."
        />
      </Modal>
    );
  }
