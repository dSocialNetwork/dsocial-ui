...
export default class PowerUpOrDown extends React.Component {
  ...
  getAvailableBalance = () => {
    const { user, down, totalVestingShares, totalVestingFundDPay } = this.props;
  ...
  handleContinueClick = () => {
    const { form, user, down, totalVestingShares, totalVestingFundDPay } = this.props;
    form.validateFields({ force: true }, (errors, values) => {
      const vests = (
        values.amount / formatter.vestToDPay(1, totalVestingShares, totalVestingFundDPay)
      ).toFixed(6);
      if (!errors) {
        const transferQuery = down
          ...
          : {
              amount: `${parseFloat(values.amount).toFixed(3)} BEX`,
              to: user.name,
            };

        const win = window.open(
          DPayID.sign(down ? 'withdraw-vesting' : 'transfer-to-vesting', transferQuery),
          '_blank',
        );
        win.focus();
