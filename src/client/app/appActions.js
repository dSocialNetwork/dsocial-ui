...
export const getRate = () => (dispatch, getState, { dpayAPI }) => {
  dispatch({
...
export const getRewardFund = () => (dispatch, getSelection, { dpayAPI }) =>
  dispatch({
    type: GET_REWARD_FUND,
    payload: { promise: dpayAPI.sendAsync('get_reward_fund', ['post']) },
  });

export const getTrendingTopics = () => (dispatch, getState, { dpayAPI }) => {
  dispatch({
    type: GET_TRENDING_TOPICS,
    payload: {
      promise: dpayAPI.sendAsync('get_trending_tags', [undefined, 50]).then(result =>
