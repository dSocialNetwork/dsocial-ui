...
export const followUser = username => (dispatch, getState, { dPayIdAPI }) => {
  ...
  return dispatch({
    type: FOLLOW_USER,
    payload: {
      promise: dPayIdAPI.follow(getAuthenticatedUserName(state), username),
    },
    meta: {
      username,
    },
...
export const unfollowUser = username => (dispatch, getState, { dPayIdAPI }) => {
  const state = getState();
  ...
  return dispatch({
    type: UNFOLLOW_USER,
    payload: {
      promise: dPayIdAPI.unfollow(getAuthenticatedUserName(state), username),
    },
    meta: {
      username,
    },
...
export const getNotifications = username => (dispatch, getState, { dsocialAPI }) => {
  const state = getState();
  ...
  return dispatch({
    ...
    meta: targetUsername,
    payload: {
      promise: dsocialAPI.sendAsync('get_notifications', [targetUsername]),
    },
  });
};
